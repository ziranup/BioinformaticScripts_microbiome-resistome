...
# 设置工作目录和数据库路径
MAGS_DIR="./MAGs_for_kraken"
OUTPUT_BASE="./MAGs_for_kraken_result"
...
process_mag() {
    ...
    # # Bracken 校正（门水平，read length=150）
    # bracken \
    #     -d "${KRAKEN_DB}" \
    ...
    # # Bracken 校正（纲水平，read length=150）
    # bracken \
    #     -d "${KRAKEN_DB}" \
    #     -i "${OUTPUT_BASE}/kraken2/${base_name}.kraken2.report" \
    #     -o "${OUTPUT_BASE}/bracken/${base_name}.kraken2.bracken" \
    ...
    # # Bracken 校正（目水平，read length=150）
    # bracken \
    #     -d "${KRAKEN_DB}" \
    #     -i "${OUTPUT_BASE}/kraken2/${base_name}.kraken2.report" \
    #     -o "${OUTPUT_BASE}/bracken/${base_name}.kraken2.bracken" \
    ...
    #     -r 150 \
    #     -l O
    # Bracken 校正（科水平，read length=150）
    bracken \
        -d "${KRAKEN_DB}" \
        -i "${OUTPUT_BASE}/kraken2/${base_name}.kraken2.report" \
        -o "${OUTPUT_BASE}/bracken/${base_name}.kraken2.bracken" \
        ...
        -r 150 \
        -l F
    # # Bracken 校正（属水平，read length=150）
    # bracken \
    #     -d "${KRAKEN_DB}" \
    #     -i "${OUTPUT_BASE}/kraken2/${base_name}.kraken2.report" \
    #     -o "${OUTPUT_BASE}/bracken/${base_name}.kraken2.bracken" \
